...
export default function OpeningsPage() {
  ...
  const loadMoreOpenings = useCallback(async () => {
    ...
    try {
      ...
      const pageSize = loadStrategy === 'medium' ? 150 : 150
  ...
  const getWinRateColor = (whiteWins: number, blackWins: number, draws: number) => {
    ...
    if (whiteRate > 55) return 'text-green-400'
    if (whiteRate > 45) return 'text-yellow-400'
    return 'text-red-400'
  }
    ...
    switch (sortBy) {
      case 'white_success':
        return { 
          ...
          label: 'White wins',
          color: stats.whiteRate > 50 ? 'text-green-400' : 'text-red-400'
        }
      case 'black_success':
        return { 
          value: `${stats.blackRate.toFixed(1)}%`, 
          label: 'Black wins',
          color: stats.blackRate > 40 ? 'text-green-400' : 'text-red-400'  
        }
      case 'balanced':
        ...
        return { 
          ...
          label: 'Balance',
          color: balance < 5 ? 'text-green-400' : 'text-yellow-400'
        }
      case 'popularity':
        return { 
  ...
  const getStrategyInfo = () => {
    switch (loadStrategy) {
      case 'small': return { icon: '⚡', text: 'All loaded', color: 'text-green-400' }
      case 'medium': return { icon: '🚀', text: 'Smart loading', color: 'text-blue-400' }
      case 'large': return { icon: '📚', text: 'Paginated', color: 'text-purple-400' }
    }
  }
      ...
      <div className="min-h-screen bg-[#0f0f0f] text-white">
        <div className="max-w-7xl mx-auto px-6 py-12">
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-gray-300 mb-2">Failed to load openings</h3>
  ...
  return (
    <div className="min-h-screen bg-[#0f0f0f] text-white">
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-white mb-2">Chess Openings</h1>
          <p className="text-gray-400">Explore and learn chess opening theory</p>
          
          {/* Strategy indicator */}
          {!loading && (
            <div className="mt-3 inline-flex items-center text-sm bg-gray-900/50 backdrop-blur-sm border border-gray-800 rounded-full px-3 py-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 gap-4 mb-8">
          ...
          >
            <div className="font-semibold text-white">All Categories</div>
            <div className="text-sm text-gray-400">{totalCount} openings</div>
          </button>
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  <span className="font-semibold text-white">ECO {category.category}</span>
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-3 border border-gray-700 bg-gray-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent text-white placeholder-gray-500"
              />
            </div>
            ...
            <div className="md:w-64">
              <div className="relative">
                <select
                  ...
                  className="w-full appearance-none bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 pr-10 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent text-white"
                >
            ...
            <div className="flex items-center space-x-6 text-sm text-gray-400">
              ...
              {loadStrategy !== 'small' && !searchQuery && (
                <div className="flex items-center space-x-2">
                  ...
                  />
                  <label htmlFor="infiniteScroll" className="text-sm text-gray-400 cursor-pointer">
                    Auto-load on scroll
                  </label>
                </div>
              )}
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-gray-300 mb-2">No openings found</h3>
            ...
            {filteredOpenings.map((opening) => {
              ...
              return (
                <Link
                  ...
                  className="block bg-gray-900/50 backdrop-blur-sm border border-gray-800 rounded-2xl p-6 hover:border-gray-700 transition-all"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <div className="flex items-center space-x-3 mb-2">
                        <h3 className="text-lg font-semibold text-white">{opening.name}</h3>
                        <span className="bg-green-900/30 text-green-400 px-2 py-1 rounded text-sm font-mono">
                          {opening.ecoCode}
                        </span>
                      ...
                      <div className="font-mono text-sm text-gray-500 bg-gray-800/50 rounded p-2">
                        {opening.moves}
                      </div>
                    </div>
        ...
        {!loading && !searchQuery && hasMore && loadStrategy !== 'small' && !infiniteScroll && (
          <div className="text-center mt-8">
            <button
              ...
              className="bg-green-600 text-white px-8 py-3 rounded-xl hover:bg-green-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center mx-auto"
            >
              {loadingMore ? (
                <>
        ...
        {!hasMore && openings.length > 0 && loadStrategy !== 'small' && (
          <div className="text-center mt-8">
            <div className="inline-flex items-center text-sm text-gray-500 bg-gray-800 px-4 py-2 rounded-full">
              <BookOpen className="h-4 w-4 mr-2" />
              All openings loaded
            </div>
          </div>
